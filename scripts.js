...
function getCategoryName(category) {
  const categories = {
      ...
      'allgemein': 'Allgemein',
      'steuerrecht': 'Steuerrecht', // Added for new category
      'arbeitsrecht': 'Arbeitsrecht' // Added for new category
  };
...
function createPodcastCards(podcasts) {
  ...
  podcasts.forEach((podcast, index) => {
      ...
      if (!podcast.title || !podcast.category || podcast.rating === undefined) {
          console.warn('Skipping invalid podcast:', podcast);
          return;
      }
      ...
      card.innerHTML = `
          ...
          <div class="podcast-card__content">
              ...
              <p class="podcast-card__description">${podcast.description || 'Keine Beschreibung verfügbar'}</p>
              ${streamingButtonsHtml}
              <div class="podcast-card__recommendation">
                  <i class="fas fa-book"></i>
                  <span>Empfohlen: <a href="${podcast.recommendLink || '#'}" target="_blank">${podcast.recommend || 'Keine Empfehlung'}</a></span>
              </div>
          </div>
  ...
  checkNoResults();
  console.log('Created cards for:', podcasts.length, 'podcasts');
}
...
// Improved function to check if results are present (No-Results-Check)
...
function resetFilters() {
  ...
  reloadPodcasts(); // Use reload to fetch updated data
}
...
// Reload podcast data manually
function reloadPodcasts() {
  const grid = document.getElementById('podcastGrid');
  grid.innerHTML = '<div class="loading">Podcasts werden neu geladen...</div>';
  ...
  fetch('podcasts.json')
      ...
      .then(data => {
          ...
          console.log('Podcasts reloaded successfully:', data.length, 'items');
      })
      .catch(error => {
          console.error('Error reloading podcasts:', error);
          grid.innerHTML = '<p>Fehler beim Laden der Podcasts. Bitte überprüfe die JSON-Datei oder lade die Seite neu.</p>';
      });
}

// Initialize page with fetch
document.addEventListener('DOMContentLoaded', function() {
  reloadPodcasts(); // Initial load with reload function
  ...
  // Add a reload button (optional, for testing)
  const reloadButton = document.createElement('button');
  reloadButton.textContent = 'Podcasts neu laden';
  reloadButton.className = 'action-button';
  reloadButton.addEventListener('click', reloadPodcasts);
  document.querySelector('.filter-actions').appendChild(reloadButton);
});